...
if [[ "${installList[@]}" =~ Security || 
        "${installList[@]}" =~ macOS ]]; then
...
if [[ "$restartRequired" == "yes" ]]; then
	...
	# Reboot notification popup
	# sudo /Library/Application\ Support/JAMF/bin/jamfHelper.app/Contents/MacOS/jamfHelper -windowType utility -title "Message from Tradeshift IT" -description "One or more recently installed updates requires you to reboot your computer. Please save all your open work and reboot now." -icon /System/Library/CoreServices/CoreTypes.bundle/Contents/Resources/AlertCautionIcon.icns -iconSize 96 -button1 "OK" -defaultButton 1
	sudo /usr/local/bin/jamf policy -event secureReboot
else
